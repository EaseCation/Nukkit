...
public class NewJungleTree extends TreeGenerator {
    ...
    public boolean generate(ChunkManager worldIn, NukkitRandom rand, BlockVector3 vectorPosition) {
        ...
        if (position.getY() > heightRange.getMinY() && position.getY() + i + 1 <= heightRange.getMaxY()) {
            ...
            if (!flag) {
                ...
                if ((block == Block.GRASS_BLOCK || block == Block.DIRT || block == Block.FARMLAND) && position.getY() < heightRange.getMaxY() - i - 1) {
    ...
    private void placeCocoa(ChunkManager worldIn, int age, BlockVector3 pos, BlockFace side) {
        ...
        this.setBlockAndNotifyAdequately(worldIn, pos, Block.get(Block.COCOA, meta));
    }
    ...
    private boolean isAirBlock(ChunkManager level, BlockVector3 v) {
        return level.getBlockIdAt(0, v.x, v.y, v.z) == 0;
    }
