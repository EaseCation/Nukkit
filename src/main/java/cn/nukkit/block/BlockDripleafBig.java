...
public class BlockDripleafBig extends BlockTransparent implements Faceable {
    ...
    public boolean place(Item item, Block block, Block target, BlockFace face, float fx, float fy, float fz, Player player) {
        Block below = down();
        int id = below.getId();
        if (!(id == BIG_DRIPLEAF || id == GRASS_BLOCK || id == DIRT || id == COARSE_DIRT || id == MYCELIUM || id == PODZOL || id == FARMLAND || id == DIRT_WITH_ROOTS || id == MOSS_BLOCK || id == PALE_MOSS_BLOCK  || id == CLAY || id == MUD || id == MUDDY_MANGROVE_ROOTS)) {
            return false;
        }
