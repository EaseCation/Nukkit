    ...
    protected void scatterVegetation(Function<Random, Block> blockProvider) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (id != CRIMSON_NYLIUM && id != WARPED_NYLIUM && id != SOUL_SOIL
                    && id != GRASS_BLOCK && id != DIRT && id != COARSE_DIRT && id != PODZOL && id != MYCELIUM && id != FARMLAND && id != DIRT_WITH_ROOTS && id != MOSS_BLOCK && id != PALE_MOSS_BLOCK && id != MUD && id != MUDDY_MANGROVE_ROOTS) {
                continue;
            }
