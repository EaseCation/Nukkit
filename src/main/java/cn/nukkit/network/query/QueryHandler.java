...
public class QueryHandler {
    ...
    public QueryHandler() {
        ...
        log.info(this.server.getLanguage().translate("nukkit.server.query.info", port));

        this.regenerateToken();
        this.lastToken = this.token;
        this.regenerateInfo();
        log.info(this.server.getLanguage().translate("nukkit.server.query.running", addr, port));
    }
    ...
    public void regenerateToken() {
        ...
        for (int i = 0; i < 16; i++) {
            token[i] = (byte) random.nextInt(255);
        }
        this.token = token;
    }
