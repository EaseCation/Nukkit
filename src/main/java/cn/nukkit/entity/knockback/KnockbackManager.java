...
public class KnockbackManager {
    ...
    private KnockbackManager() {
        ...
        // 完全覆盖式：关闭旧动量继承，击退为纯固定向量（方向=攻击反向、大小=定死值），
        // 既不读 motionXYZ 残留也不读 player.speed，连击不累加——还原 KnockbackMaster 的可预测击退本意。
        register(new KnockbackProfile("je")
                .setBaseH(0.45f).setBaseV(0.35f)
                .setSprintBaseH(0.65f).setSprintBaseV(0.3f)
                .setAirMultiplierH(0.8f).setAirMultiplierV(0.8f)
                .setEnchantBonusH(0.4f).setEnchantBonusV(0.35f) // 击退附魔加成（近似 KnockbackMaster 的乘数手感）
                .setHorizontalMax(1.3f).setHorizontalMin(0.1f) // 放宽水平上限，容纳地面疾跑(0.8)+击退附魔，避免方向修正截断
                .setInheritHorizontal(false).setInheritVertical(false) // 完全覆盖，不叠加玩家任何真实速度
                .setStopSprinting(true) // 还原 Java：疾跑攻击命中后取消攻击者疾跑（W-tap 基础）
                .setSprintSlowdownH(0.6f) // 还原 Java：攻击者命中后水平速度降至 60%
