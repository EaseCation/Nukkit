                ...
                if (level.getHeightRange().isValidBlockY(y)) {
                    for (int x = boundingBox.getFloorMinX(); x <= boundingBox.getFloorMaxX(); x++) {
                        for (int z = boundingBox.getFloorMinZ(); z <= boundingBox.getFloorMaxZ(); z++) {
                            ...
                            if (block.isAir() || !block.collide(boundingBox.getOffsetBoundingBox(0, -0.026, 0))) {
                                continue;
                            }
...
//                    }
    ...
    public void onPlayerInput(Player player, double motionX, double motionY) {
        ...
        deltaRotation = Mth.clamp(deltaRotation, -5, 5);
        yaw += deltaRotation;
        ...
        float rad = ((float) yaw - 90) * Mth.DEG_TO_RAD;
        this.motionX += Mth.sin(-rad) * acceleration;
        this.motionZ += Mth.cos(rad) * acceleration;
    }
